...
class UnifiedMedicalDataset(Dataset):
    ...
    def should_take_file(self, path, label_name):
        ...
        if label_name == "mrt_body":

            if "spine mri dataset" in lower_path:
            ...
            if "automatedcardiacdiagnosischallenge_miccai17" in lower_path:
                ...
                if suffix == ".nii":
                    return "_frame" in name

                return False

            if "kneemridataset" in lower_path:
                return suffix == ".pck"
